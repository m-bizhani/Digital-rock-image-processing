...
def wdsr(scale, num_filters, num_res_blocks, res_block_expansion, res_block_scaling, res_block):
    ...
    m = conv2d_weightnorm( scale ** 2, 3, padding='same', name=f'conv2d_main_scale_{scale}')(m)
    m = Lambda(pixel_shuffle(scale))(m)

    # skip branch
    s = conv2d_weightnorm( scale ** 2, 5, padding='same', name=f'conv2d_skip_scale_{scale}')(x)
...
if __name__ == '__main__':
    model = wdsr_a(scale=4, num_filters=32, num_res_blocks=8, res_block_expansion=4, res_block_scaling=None)
    print(model.summary())